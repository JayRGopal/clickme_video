...
data_dir = '/home/jgopal/jgopal/clickme_v2/images/' 
model_dir = '/home/jgopal/jgopal/models/saliency_volcanic_monkey.h5'
...
def load_im(im_name, data_dir=data_dir, final_size=(256, 256)):
...
def load_guesser():
    ...
    #import pdb;pdb.set_trace()
    guesser.batch_shape =  guesser._build_input_shape.as_list()#.get_shape().as_list()
    guesser.input_batch = np.zeros(guesser.batch_shape)
    guesser.feed_dict = {guesser.input: guesser.input_batch}
    guesser.prob = tf.convert_to_tensor(guesser.predict(guesser.input_batch))
    # Prepare image cache
    #guesser.image_cache = ImageCache(dc_data_dir, input_size=(256, 256, 3), crop_size=guesser.batch_shape[1:3])
    guesser.image_cache = ImageCache(dc_data_dir, input_size=(224, 224, 3), crop_size=guesser.batch_shape[1:3])
...
def get_image_prediction(guesser, image_name, clicks, click_size=21): # TODO: Using a larger click size for debugging
    ...
    crop_offset = [(i - c) / 2 for (i, c) in zip((256, 256), input_shape[1:3])]
    ...
    #prob = guesser.predict(tf.convert_to_tensor(guesser.input_batch), batch_size=1,verbose='auto',steps=1)
